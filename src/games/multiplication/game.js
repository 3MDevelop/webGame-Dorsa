...
function fQuestion() {
  ...
  for (let index = 0; index < (cellCount / 2); index++) {
    val1 = randRange(gData.gameDef[window.gameCurLevel].numDLin1, gData.gameDef[window.gameCurLevel].numULin1, 1)
    val2 = randRange(gData.gameDef[window.gameCurLevel].numDLin2, gData.gameDef[window.gameCurLevel].numULin2, 1)
    multi = val1 * val2
    if (isValInArr(qArrCH, multi)) {
      index--
    } else {
      qArrCH[index] = multi
      qArr[index] = [multi, val1 + " * " + val2]
    }
  }
