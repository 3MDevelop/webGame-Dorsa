...
const aClick = (n) => () => window.aDivClick(n)
...
class GameUI extends React.Component {
  render() {
    let cellArr = Array.from({ length: 9 }, (_, i) => i + 1)
    return (
      <div className="game">
        <div className="first" onClick={qClick()}></div>
        <div className="second">
          {cellArr.map((cellArr) => <Box key={cellArr} id={cellArr} />)}
        </div>
      </div>
    )
  }
